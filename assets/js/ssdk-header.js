...
window.updateHeaderAuth = (user) => {
  ...
  if (authBtn) {
    if (user) {
      ...
      const name = user.user_metadata?.full_name || (user.email ? user.email.split("@")[0].substring(0, 8) : "User");
      authBtn.textContent = name + " (Dash)";
      authBtn.style.border = "1px solid var(--accent, #C084FC)";
      authBtn.style.color = "var(--accent, #C084FC)";
    } else {
...
// Listen for global auth changes from SupabaseEngine
window.addEventListener("ssdk-auth-change", (e) => {
  window.updateHeaderAuth(e.detail);
});

// Check initial session via Supabase JS if available
if (window.location.protocol !== 'file:') {
  import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js/+esm').then(({ createClient }) => {
    const SUPABASE_URL = "https://wyqdfwtslkfzmorvggdq.supabase.co";
    const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Ind5cWRmd3RzbGtmem1vcnZnZ2RxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODM3NzQ5MTYsImV4cCI6MjA5OTM1MDkxNn0.I1CPWLqtv-p3XBC2F_6f-IEBbr0M_G6JkB76vc1ZR8A";
    const client = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
    client.auth.getSession().then(({ data: { session } }) => {
      if (session && session.user) {
        window.updateHeaderAuth(session.user);
      }
    }).catch(() => {});
  }).catch(() => {});
}
