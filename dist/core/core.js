...
import { FeatureEngine } from "../engines/feature-engine.js";

export class CoreEngine {
  constructor() {
    this.prefix = this.determineDepth();
    this.config = new ConfigEngine(this.prefix);
    this.feature = new FeatureEngine();
  ...
  async init() {
    ...
    const criticalEngines = [
      ...
      "feature",      // Feature Flag & SaaS Permissions
      "category",     // Category metadata manager
      "capability",   // Tool capabilities adaptive UI
      "a11y",         // Accessibility Engine
      "theme",        // Centralized Theme Manager
      "search",       // Indexing
    ...
    for (const name of criticalEngines) {
      ...
      if (engine && typeof engine.init === "function") {
        try {
          ...
          console.error(`[CoreEngine] Critical Sub-engine [${name}] failed to initialize:`, e);
    ...
    // Lazy initialization for non-blocking engines
    setTimeout(async () => {
      for (const name of deferredEngines) {
        const engine = this.engines[name];
        if (engine && typeof engine.init === "function") {
          try {
            const start = performance.now();
            ...
            console.warn(`[CoreEngine] Deferred Sub-engine [${name}] failed to initialize:`, e);
          }
        }
      }
    }, 500);
