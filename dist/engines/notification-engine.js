...
export class NotificationEngine {
   ...
   */
  async showSystemBanner() {
    const config = this.core.getEngine("config");
    if (!config) return;

    const siteSettings = await config.loadJSON("site.json");
    if (siteSettings && siteSettings.announcement && siteSettings.announcement.active) {
      const { text, type, id } = siteSettings.announcement;
      
      // Check if dismissed
      const isDismissed = localStorage.getItem(`ssdk-banner-${id}`) === "true";
      if (isDismissed) return;

      this.injectBannerDOM(text, type, id);
    }
  }

  injectBannerDOM(text, type, id) {
    const banner = document.createElement("div");
    banner.className = `announcement-banner banner-${type}`;
    banner.style.cssText = `
      background: ${type === 'maintenance' ? 'var(--danger-color)' : 'var(--primary-color)'};
      color: #fff;
      padding: 10px 20px;
      text-align: center;
      font-size: 0.9rem;
    ...
    banner.innerHTML = `
      <span>${text}</span>
      <button style="background:transparent; border:none; color:#fff; cursor:pointer; font-weight:800; font-size:1.1rem;" onclick="this.parentElement.remove(); localStorage.setItem('ssdk-banner-${id}', 'true')">&times;</button>
    `;
